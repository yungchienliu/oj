...
void update_slackv(int v, int x){
	if (!slackv[x] || e_delta(mat[v][x]) < e_delta(mat[slackv[x]][x]))
		slackv[x] = v;
}
void calc_slackv(int x){
	slackv[x] = 0;
	for (int v = 1; v <= n; v++)
...
void add_blossom(int xv, int xa, int xu){
	int b = n + 1;
	while (b <= n_x && bel[b]) b++;
	if (b > n_x) n_x++;
	lab[b] = 0;
...
void expand_blossom1(int b){ // lab[b] == 1
	for (int i = 0; i < size(bloch[b]); i++)
...
int main () {
	...
	while(~scanf("%d", &n)){
		...
		for(int i = 0; i < n; ++i)
			for(int j = 0; j < n; ++j){
				if(i == j) continue;
				mat[i+1][j+1] = edge(i+1, j+1, 0);
				mat[i+1][j+1].w = (int)(-hypot(X[i]-X[j],Y[i]-Y[j])*1e6+1e9);
			}
		calc_max_weight_match();
		double ans = tot_weight;
		printf("Case %d: %.2f\n", ++tt, -(ans-(double)n/2*1e9)/1e6);
	}
}
